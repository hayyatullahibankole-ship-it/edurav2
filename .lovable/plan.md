...
The new pages exist in the code and work on the web: `/leaderboard`, `/ai-tutor`, `/ebooks`, and `/ebooks/:slug` are all registered in the app router. So the 404 you see is not a missing page — the installed app is still running an **older copy of the app** that was saved on your phone. Its router doesn't know those addresses yet, so it falls back to the 404 screen.
...
### 3. Ebook reading fully inside the app
